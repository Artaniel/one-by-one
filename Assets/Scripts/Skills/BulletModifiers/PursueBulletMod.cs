...
public class PursueBulletMod : BulletModifier
{
    ...
    {
        UnityEngine.Collider2D[] collider2Ds = Physics2D.OverlapCircleAll(bullet.transform.position, radius);
    ...
    private void RotateToTarget(GameObject monsterTarget, BulletLife bullet)
    {
        ...
        {
            bullet.transform.rotation = Quaternion.Euler(0, 0, currentAngle + (Mathf.Sign(difference) * factorRotationSpeed * Time.deltaTime));
        }
    }
